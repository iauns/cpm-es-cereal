...
TEST(EntitySystem, SerializeOneEntity)
{
  ...
  uint64_t compID = 3;
  auto verifyDocumentStructure = [compID, rootID, &checkCompPosition, &checkCompHomPos, &checkCompGameplay](Tny* doc)
  {
    ...
    ASSERT_EQ(TNY_OBJ, doc->type);
    {
...

      {
        ...
        ASSERT_EQ(2, comp->size);   // int64 and object.
  ...
  Tny* root = core->serializeEntity(compID);
